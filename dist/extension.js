...
async function generateResponse(userInput, selectedText) {
  try {
    const fullInput = selectedText ? `Hi Darling~! Here's the code:

${selectedText}

You: ${userInput}` : userInput;
    const response = await fetch2("http://localhost:8000/ai/chat", {
      method: "POST",
    ...
    const parsed = data;
    return parsed.response || "UwU~ Something went wrong, sowwy~ \u{1F494}";
  } catch (error) {
    return "Oopsies~ I couldn't reach my backend brain \u{1F622} Make sure it's running!";
  }
}
